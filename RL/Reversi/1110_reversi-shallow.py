'''1110 reversi-shallow neural network
...
class Game:
    cpPath = "shallow/cp_{0:06}.ckpt" # 저장된 가중치
    ...
    def __init__(self):
        ...
        # 강화학습을 위한 파라미터
        self.alpha = 0.1        # 학습률
        self.gamma = 0.99       # 미래가치 반영률(설레발)
                                # 미래의 가치를 현재에 얼마나 반영할 것인가

        # e-greedy(입실론 탐욕) 파라미터
        self.epsilon = 1.0          # 초기 입실론
        self.epsilonDecay = 0.999   # 입실론 값을 매번 줄여나감.(탐색보단 시행에 더 집중)
    ...
    def onQuit(self, buf):
        ...
        win = (w > b) - (w < b)
            # 자신이 white(turn1)인데 이기면 win=1, 지면 win=-1
            # 자신이 black(turn2)인데 이기면 win=-1, 지면 win=1
        # 자신이 이겼는가.
        result = win+1 if self.turn == 1 else 1-win
            # 자신이 turn1(white)인데, 이기면 result=2, 비기면 result=1, 지면 result=0
            # 자신이 turn2(black)인데, 이기면 result=2, 비기면 result=1, 지면 result=0
        
        winText = ("Lose", "Draw", "Win")
        print(f"{winText[result]} W : {w}, B : {b}")

        # 최종 보상  -  자신이 이김:1, 비김 : 0.5,  자신이 짐: 0
        reward = result/2 # 스케일링

        # 마지막 상태는 더 이상 값이 필요 없는 상태
        self.episode[-1] = (self.episode[-1][0], reward)
                # episode[-1] : 마지막(nst, v) -> (nst, reward) 로 바꿔!

        # 학습을 위해서 데이터 처리
        x, y = [], []

        # 기록된 모든 상태 에피소드를 마지막 순서부터 처음 순서 까지 확인하면서.
        for st, v in self.episode[::-1]:
            # 보상 = 0.9*기대값 + 0.1*(미래가치반영 비율 적용된 보상값)
            rw = (1-self.alpha)*v + self.alpha*reward
                # 마지막 state에서 보상값은 1로
            x.append(st)
            y.append(rw)
            reward = self.gamma*rw # 미래의 리워드를 현재에 얼마나 반영할 것인지(0.99)

        # 에피소드 값을 이용하여 리플레이를 하도록 한다.
        self.replay(x,y)
        return result # 자신이 이김 : 2, 비김 : 1, 자신이 짐 : 0

    def onBoard(self, buf):
        nst, p, v = self.action(buf)
        if p < 0: return False # 놓을 수 있는 위치를 반환 받아야함.
        self.send("%04d pt %04d"%(8, p))
        self.episode.append((nst, v)) # 자신이 두었을 때의 상태(다음상태)를 기록 + 그때의 기대값.
