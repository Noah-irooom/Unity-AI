...
class Reversi:
    ...
    # ex) join Noah : 리버시 판에 참여한다.
    def onJoin(self, ss):
        if self.players[1] == None:
            self.players[1] = ss[1] 
            ret = "white"           # action Reversi join white
            self.players[0] += 1
        elif self.players[2] == None:
            self.players[2] = ss[1]
            ret = "black"
            self.players[0] += 1
        else: return "refuse"       # action Reversi join refuse
    ...
    def onLeave(self, ss):
        # ss[1]이 플레이어에 있는지 검사.
        for i in range(1, 3):
            if self.players[i] == ss[1]:    # 그만두고자 하는 플레이어찾
                self.players[i] = None
                self.players[0] -= 1
    ...
    def runCommand(self, s):
        ss = s.split()
        if ss[0] == "join": return self.onJoin(ss)      # join Noah
        if ss[0] == "board": return self.onBoard()
        if ss[0] == "place": return self.onPlace(ss)
        if ss[0] == "leave": return self.onLeave(ss)    # leave Noah
